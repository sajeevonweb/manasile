...
import { QUESTIONS_PER_PAGE, calculateShortScores } from '../data/short';
import { getShortQuestions } from '../translations';
...
export default function ShortAssessment() {
  ...
  const questions = useMemo(() => 
    getShortQuestions((assessment?.language as Language) || 'en'), 
    [assessment?.language]
  );
  ...
  useEffect(() => {
    const loadAssessment = async () => {
      ...
      try {
        ...
        const questionsForLanguage = getShortQuestions(
        ...
        if (assessmentData.responses && Object.keys(assessmentData.responses).length > 0) {
          Object.entries(assessmentData.responses).forEach(([questionId, value]) => {
            ...
            if (!isNaN(numValue) && numValue >= 1 && numValue <= 5) {
              setValue(questionId, numValue, { shouldValidate: false, shouldDirty: false });
            }
          });

          // Jump to first unanswered question
          const firstUnansweredIndex = questionsForLanguage.findIndex(q => !assessmentData.responses?.[q.id]);
          const targetPage = firstUnansweredIndex >= 0
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const calculatedScores = calculateShortScores(numericResponses, questions);
      const scores = {
        extraversion: Math.round(Math.max(0, Math.min(100, calculatedScores.extraversion))),
        agreeableness: Math.round(Math.max(0, Math.min(100, calculatedScores.agreeableness))),
        conscientiousness: Math.round(Math.max(0, Math.min(100, calculatedScores.conscientiousness))),
        neuroticism: Math.round(Math.max(0, Math.min(100, calculatedScores.neuroticism))),
        openness: Math.round(Math.max(0, Math.min(100, calculatedScores.openness))),
      };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Form */}
      <div onSubmit={currentPage === totalPages - 1 ? onFinalSubmit : (e) => e.preventDefault()}>
